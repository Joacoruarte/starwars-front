...
interface EntityMapper {
    [Entity.FILMS]: ({ page, search, controller }: GetDataFromEntityProps) => Promise<FilmResponse>;
    [Entity.CHARACTERS]: ({ page, search, controller }: GetDataFromEntityProps) => Promise<CharacterResponse>;
    [Entity.PLANETS]: ({ page, search, controller }: GetDataFromEntityProps) => Promise<PlanetResponse>;
    [Entity.STARSHIPS]: ({ page, search, controller }: GetDataFromEntityProps) => Promise<StarshipResponse>;
}

type EntityResponse = FilmResponse | CharacterResponse | PlanetResponse | StarshipResponse;

export const getEntitysData = async ({ entity, page, search, controller }: GetEntitysDataProps): Promise<EntityResponse> => {
